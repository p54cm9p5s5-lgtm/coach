...
export async function aggiornaProposte(cache = null) {
  ...
  // Sul Mac il motore non gira: proposte e segnali li calcola l'iPhone e
  // arrivano con la sincronizzazione. Calcolate su tutti e due, la stessa
  // proposta nascerebbe due volte con due id diversi (js/db.js).
  if (db.ilMotoreGiraAltrove()) return { create: 0, tolte: 0 };
  const oggi = isoDate();
  const reg = regole();
...
export async function aggiornaSegnali(cache = null) {
  if (db.ilMotoreGiraAltrove()) return (await db.all("segnali")).length; // vedi aggiornaProposte
